# We add the interaction between the head and the layers
...
class dep_control_nn(nn.Module):
    def __init__(self,
                 hidden_size, gate_type='d'):
        super().__init__()
        self.gate_type = gate_type
        self.w_xz = nn.Linear(hidden_size, hidden_size)
        self.w_hz = nn.Linear(hidden_size, hidden_size)
        self.sigmoid_1 = nn.Sigmoid()
        if gate_type == 'd':
            self.w_xr = nn.Linear(hidden_size, hidden_size)
            self.w_hr = nn.Linear(hidden_size, hidden_size)
            self.sigmoid_2 = nn.Sigmoid()

    def forward(self, x, h):
        z = self.sigmoid_1(self.w_xz(x) + self.w_hz(h))
        if self.gate_type == 'd'
            r = self.sigmoid_2(self.w_xr(x) + self.w_hr(h))
            output = z * h + r * x
        else:
            output = (1 - z) * x + z * h
        return output
...
class MultiHead_Attention_Lattice_rel_save_gpumm(nn.Module):
    def __init__(self, hidden_size, num_heads, pe,
                 ...
                 scaled=True, mode=collections.defaultdict(bool),max_seq_len=-1,
    ...
    def forward(self,key, query, value, mask_use, seq_len, lex_num, pos_s,pos_e,rel_pos_embedding):
        ...
        if mask_use != None:
            if len(mask_use.size()) == 2:
                attn_score_raw_masked = attn_score_raw_masked.masked_fill(mask_use.unsqueeze(1).unsqueeze(1), -1e15)
            else:
                attn_score_raw_masked = attn_score_raw_masked.masked_fill(mask_use.unsqueeze(1), -1e15)
        ...
        return result, attn_score_raw_masked
...
class Positionwise_FeedForward(nn.Module):
    def __init__(self, sizes, dropout=None,ff_activate='relu'):
        ...
        for i in range(self.num_layers):
            setattr(self, 'w_feedforward_NN' + str(i), nn.Linear(sizes[i], sizes[i + 1]))
    ...
    def forward(self, inp):
        ...
        for i in range(self.num_layers):
            ...
            w = getattr(self, 'w_feedforward_NN' + str(i))
            output = w(output)
            if i == 0:
...
class Transformer_Encoder_Layer(nn.Module):
    def __init__(self, hidden_size, num_heads,
        ...
        self.max_seq_len = max_seq_len
        self.dvc = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        ...
        print(self.require_ffn)
        print(self.require_ffn==1)
        if self.require_ffn == 1:
            print('*' * 90)
            print(self.require_ffn)
            print("In this layer, we use feedforward Nerual NetWork")
            self.ff = Positionwise_FeedForward([hidden_size, ff_size, hidden_size], self.dropout,ff_activate=self.ff_activate)

    def forward(self, inp, seq_len, mask_use, 
                total_length=None, dep_matrix=None, lex_num=0,pos_s=None,pos_e=None,
                rel_pos_embedding=None):
        mask_use_ = None
        if total_length != None:
            mask_use_ = torch.zeros(seq_len.size(0), total_length).bool().to(self.dvc)
            if mask_use == 'dep_and_adj':
                batch = dep_matrix.size(0)
                mask_use_ = []
                for i in range(batch):
                    indice = dep_matrix[i]
                    value = torch.full([indice.size(0)], 1).to(self.dvc)
                    mask = ~(torch.sparse.FloatTensor(indice.t(), value, torch.Size([total_length+10, total_length+10])).to_dense().bool())
                    mask = mask[:total_length, :total_length]
                    mask_use_.append(mask)
                mask_use_ = torch.stack(mask_use_)
            mask_use_ = mask_use_.to(self.dvc)
        output = inp
        ...
        if self.lattice:
            if self.relative_position:
                ...
                output, attn_score = self.attn(output, output, output, mask_use_, seq_len, pos_s=pos_s, pos_e=pos_e, lex_num=lex_num,
                                   rel_pos_embedding=rel_pos_embedding)
            else:
                output, attn_score = self.attn(output, output, output, seq_len, lex_num)
        else:
            output, attn_score = self.attn(output, output, output, seq_len, pre_attn=pre_attn)
...
class Transformer_Encoder(nn.Module):
    def __init__(self, hidden_size, num_heads, num_layers,
                 relative_position, learnable_position, add_position,
                 layer_preprocess_sequence, layer_postprocess_sequence,
                 dropout=None, scaled=True, 
                 ff_size=-1, masks=None,
                 require_ffn=None,
                 use_dep_control=0,
        ...
        if use_dep_control == 1:
            self.dep_control_nn = dep_control_nn(hidden_size, use_dep_control)
        self.require_ffn = require_ffn
        ...
        for i in range(self.num_layers):
            setattr(self, 'layer_{}'.format(i),Transformer_Encoder_Layer(hidden_size, num_heads,
                                                    relative_position, learnable_position, add_position,
                                                    layer_preprocess_sequence, layer_postprocess_sequence,
                                                    self.require_ffn[i],dropout,scaled,ff_size,
                                                    mode=self.mode,
                                                    max_seq_len=self.max_seq_len,
                                                    pe=self.pe,
                                                    pe_ss=self.pe_ss,
    ...
    def forward(self, inp, seq_len,total_length, lex_num=0,pos_s=None,pos_e=None, dep_matrix=None):
        output = inp
        if self.relative_position:
        ...
        for i in range(self.num_layers):
            if self.masks != None:
                mask_use = self.masks[i]
            else:
                mask_use = None
            now_layer = getattr(self, 'layer_{}'.format(i))
            if not mask_use == 'dep_and_adj':
                dep_matrix = []
                output, score = now_layer(output, seq_len, mask_use, total_length, lex_num=lex_num, pos_s=pos_s, pos_e=pos_e,
                                   rel_pos_embedding=rel_pos_embedding, dep_matrix=dep_matrix)
            else:
                output, score = now_layer(output, seq_len, mask_use, total_length, lex_num=lex_num, pos_s=pos_s, pos_e=pos_e,
                                   rel_pos_embedding=rel_pos_embedding,  dep_matrix=dep_matrix)
            if self.use_dep_control and i == 0:
                output = self.dep_control_nn(inp, output)
        output = self.layer_preprocess(output)
