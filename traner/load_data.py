...
@cache_results(_cache_fp='cache/load_yangjie_rich_pretrain_word_list',_refresh=True)
...
@cache_results(_cache_fp='cache/seg_vocab',_refresh=True)
def load_dataset_seg_vocab(seg_vocab_path, drop_characters=True):
    f = open(seg_vocab_path)
    lines = f.readlines()
    seg_vocab = []
    for line in lines:
        seg_vocab.append(line.strip('\n'))

    if drop_characters:
        seg_vocab = list(filter(lambda x: len(x) != 1, seg_vocab))
    return seg_vocab
